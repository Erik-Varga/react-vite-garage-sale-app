        ...
        <div>
            <div className="py-5 flex justify-center items-center gap-2">
                ...
                <h1 className="text-xl text-gray-600 font-bold">
                    {getAllItems.length}x Total Items
                </h1>
                {/* add item button */}
                <Link to={'/add-item'}>
                    <button className='text-white px-5 py-2 bg-gray-500 border hover:bg-gray-600 border-gray-100 rounded-lg cursor-pointer'>Add Item</button>
                </Link>
            </div>
            ...
            {/* antd table */}
            <div>
                <Table className='flex justify-center' dataSource={getAllItems} columns={columns} size='small' bordered sorter pagination={{ defaultPageSize: 25, showSizeChanger: true, pageSizeOptions: ['25', '50', '100'] }} />
            </div>
        </div>
    )
}
